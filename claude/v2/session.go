...
func (s *v2SessionImpl) McpServerStatus(ctx context.Context) ([]shared.McpServerStatus, error) {
	...
	if client, ok := s.client.(interface {
		McpServerStatus(context.Context) ([]shared.McpServerStatus, error)
	}); ok {
		return client.McpServerStatus(ctx)
	}
...
func (s *v2SessionImpl) SetMcpServers(ctx context.Context, servers map[string]shared.McpServerConfig) (*shared.McpSetServersResult, error) {
	// Forward to underlying client
	if client, ok := s.client.(interface {
		SetMcpServers(context.Context, map[string]shared.McpServerConfig) (*shared.McpSetServersResult, error)
	}); ok {
