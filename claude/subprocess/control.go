...
// This file implements the control protocol for bidirectional communication.
package subprocess
...
// ControlTransport abstracts the I/O operations for the control protocol.
...
type ControlTransport interface {
	...
	// For the real transport, this returns a closed channel since we route directly.
...
type Protocol struct {
	...
	// Hook callbacks
	hooks            map[shared.HookEvent][]ProtocolHookMatcher
	hookCallbacks    map[string]ProtocolHookCallback
...
func WithCanUseToolCallback(callback shared.CanUseToolCallback) ProtocolOption {
	return func(p *Protocol) {
...
func WithHooks(hooks map[shared.HookEvent][]ProtocolHookMatcher) ProtocolOption {
	return func(p *Protocol) {
		p.hooks = hooks
	}
}
...
func WithHookCallbacks(callbacks map[string]ProtocolHookCallback) ProtocolOption {
	return func(p *Protocol) {
...
func WithSdkMcpServers(servers map[string]*mcp.SdkMcpServer) ProtocolOption {
	return func(p *Protocol) {
...
func NewProtocol(transport ControlTransport, opts ...ProtocolOption) *Protocol {
	p := &Protocol{
		transport:       transport,
...
func (p *Protocol) Start(ctx context.Context) error {
	...
	// Start background message reader if transport provides a read channel
	readChan := p.transport.Read(p.ctx)
	if readChan != nil {
		p.wg.Add(1)
		go p.readLoop(readChan)
	}
...
func (p *Protocol) readLoop(readChan <-chan []byte) {
	defer p.wg.Done()
...
// generateRequestID creates a unique request ID matching TypeScript SDK format.
...
// Returns error if the control request fails or times out.
//
// This method matches TypeScript SDK's rewind_files behavior exactly:
...
// IsInitialized returns whether the protocol has completed initialization.
...
func (p *Protocol) SetPendingRequest(requestID string, responseChan chan *ControlResponse) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.pendingRequests[requestID] = responseChan
}
