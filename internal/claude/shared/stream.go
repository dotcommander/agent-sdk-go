...
func FormatStats(stats StreamStats) string {
	...
	parts = append(parts, fmt.Sprintf("Total messages: %d", stats.TotalMessages))

	if stats.PartialMessages > 0 {
		parts = append(parts, fmt.Sprintf("Partial messages: %d", stats.PartialMessages))
	}

	if stats.Errors > 0 {
		parts = append(parts, fmt.Sprintf("Errors: %d", stats.Errors))
	}

	if stats.ProcessingTime != "" {
		parts = append(parts, fmt.Sprintf("Processing time: %s", stats.ProcessingTime))
	}
